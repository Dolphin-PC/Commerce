...
describe("제품 목록 조회 | 성공", async () => {
  it("기본 조회", async () => {
    const { result } = renderHook(
      () => useProductListCategoryInfiniteQuery({}), // Props
  ...
  it("판매자 조회", async () => {
    ...
    const { result } = renderHook(
      () => useProductListCategoryInfiniteQuery({ sellerId }), // Props
      { wrapper: queryWrapper() }
    );
  ...
  it("카테고리 조회", async () => {
    const categoryId = 1;
    const { result } = renderHook(
      () => useProductListCategoryInfiniteQuery({ categoryId }), // Props
      { wrapper: queryWrapper() }
    );
  ...
  it("필터링", async () => {
    ...
    // when
    const { result } = renderHook(
      () => useProductListCategoryInfiniteQuery({ filter: { searchText, categoryIds, priceRange } }), // Props
      { wrapper: queryWrapper() }
    );
