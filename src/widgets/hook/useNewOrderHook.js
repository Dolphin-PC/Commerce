...
export const useNewOrderHook = () => {
    ...
    //* 장바구니에서 주문 생성
    const handleNewOrderByCart = async (cartList) => {
        return new Promise(async (resolve, reject) => {
            try {
                 ...
                 */
                // 2. 주문상세::생성 (수량이 0인 상태)
                const orderDetailCartList = await Promise.all(cartList.map(async (cart) => {
                    ...
                    return [newOrderDetail, cart];
                }));
                // 3. 상품재고 수량을 감소시키면서, 주문상세 수량 증가
                try {
                    await Promise.all(orderDetailCartList.map(async (orderDetailCart, index) => {
                    ...
                    await Promise.all(orderDetailCartList.map(async ([orderDetail, _]) => {
    ...
    //* 즉시 주문
    const handleNewOrder = async ({ userId, productId, quantity }) => {
        const order = await postNewOrder({ userId });
        const orderDetail = await postOrderDetailMutation.mutateAsync({ insert: { orderId: order.id, quantity, productId } });
